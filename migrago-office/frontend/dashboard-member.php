...
$view = sanitize_key($_GET['view'] ?? 'profits');
$allowed_views = ['profits', 'add_client', 'search'];
...
$clients = migrago_office_search_clients_for_member($search_term, get_current_user_id());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['migrago_client_nonce']) && wp_verify_nonce(sanitize_text_field($_POST['migrago_client_nonce']), 'migrago_add_client')) {
    ...
    if ($client_id > 0) {
        ...
        $message = 'تم إنشاء العميل بنجاح. رقم الملف: <strong>' . esc_html($file_number) . '</strong> | <a href="' . esc_url($receipt_url) . '" target="_blank">تحميل/طباعة التوصيل</a> | <a href="' . esc_url($contract_url) . '" target="_blank">تحميل/طباعة عقد الخدمة</a>';
    } else {
        $message = 'فشل إنشاء العميل. تحقق من البيانات ثم أعد المحاولة.';
    }
    $view = 'add_client';
}
...
<div class="mo-dashboard-shell" dir="rtl">
    <header class="mo-simple-header">مرحباً بموظفي MIGRAGO الأعزاء — شكراً لجهودكم اليومية ونتمنى لكم عملاً منظماً وناجحاً.</header>
    <div class="mo-layout">
        <aside class="mo-sidebar">
            <h2>لوحة الموظف</h2>
            <a href="<?php echo esc_url(add_query_arg(['view' => 'profits'], $base_url)); ?>" class="mo-nav-btn <?php echo $view === 'profits' ? 'is-active-nav' : ''; ?>">💵 أرباحي</a>
            <a href="<?php echo esc_url(add_query_arg(['view' => 'add_client'], $base_url)); ?>" class="mo-nav-btn <?php echo $view === 'add_client' ? 'is-active-nav' : ''; ?>">➕ عميل جديد</a>
        ...
        <main class="mo-content">
            <?php if ($view === 'profits') : ?>
                <section class="mo-panel"><h1>أرباحي</h1><p>سيتم تطوير هذا القسم لاحقًا.</p></section>
            <?php elseif ($view === 'add_client') : ?>
                <section class="mo-panel"><h1>استمارة إضافة عميل</h1><?php if ($message !== '') : ?><p class="mo-notice"><?php echo wp_kses_post($message); ?></p><?php endif; ?>
                    <form method="post" class="mo-form-grid"><?php wp_nonce_field('migrago_add_client', 'migrago_client_nonce'); ?><input name="full_name" placeholder="الاسم الكامل" required><input name="phone" placeholder="الهاتف" required><input name="email" type="email" placeholder="البريد"><input name="passport_number" placeholder="رقم جواز السفر"><input name="national_id" placeholder="رقم البطاقة الوطنية"><input name="birth_date" placeholder="تاريخ الازدياد (مثال: 24.11.2007)"><input name="birth_place" placeholder="مكان الازدياد"><textarea name="address" placeholder="العنوان الكامل"></textarea><select name="service_type"><?php foreach (migrago_office_service_types() as $type) : ?><option value="<?php echo esc_attr($type); ?>"><?php echo esc_html($type); ?></option><?php endforeach; ?></select><textarea name="service_description" placeholder="وصف الخدمة"></textarea><input name="agreed_amount" type="number" step="0.01" placeholder="المبلغ المتفق عليه"><input name="first_payment" type="number" step="0.01" placeholder="الدفعة الأولى"><select name="payment_method"><?php foreach (migrago_office_payment_methods() as $method) : ?><option value="<?php echo esc_attr($method); ?>"><?php echo esc_html($method); ?></option><?php endforeach; ?></select><button type="submit">حفظ العميل</button></form>
                </section>
            <?php else : ?>
                <section class="mo-panel"><h1>البحث المتقدم + إضافة مبلغ جديد</h1><?php if ($payment_message !== '') : ?><p class="mo-notice"><?php echo wp_kses_post($payment_message); ?></p><?php endif; ?><?php if ($edit_message !== '') : ?><p class="mo-notice"><?php echo esc_html($edit_message); ?></p><?php endif; ?>
                    <form method="get" class="mo-inline-form"><input type="hidden" name="view" value="search"><input name="q" value="<?php echo esc_attr($search_term); ?>" placeholder="ابحث بالاسم الكامل أو رقم البطاقة الوطنية أو رقم جواز السفر أو الهاتف"><button type="submit">بحث</button></form>
                    <?php if ($search_term !== '') : ?><div class="mo-notice">عدد النتائج: <?php echo esc_html((string) count($clients)); ?></div><?php endif; ?>
                    <?php if (empty($clients)) : ?>
                        <p class="mo-empty-state"><?php echo $search_term === '' ? 'ابحث بالاسم الكامل أو برقم البطاقة أو الجواز أو الهاتف.' : 'لا توجد نتائج مطابقة.'; ?></p>
                    <?php else : ?>
